...
from sh_common import decode_id, encode_id, publish_event, send_email, token_required, roles_required
...
def _send_patient_confirmation(conn, appointment: dict):
    ...
    # Cross-service notification: health-data-service (owner of the alerts
    # table) creates an in-app alert from this event. Best-effort -- a
    # missing broker must never block booking an appointment.
    publish_event("appointment.created", {
        "user_id": appointment["user_id"],
        "title": "Rendez-vous confirmé",
        "message": f"Votre rendez-vous avec Dr. {doctor_name} est prévu le {appointment_date} à {appointment_time}.",
    })
...
def _send_reminder_email(conn, appointment):
    ...
    doctor_name = appointment.get("doctor_name") or "votre médecin"
    body = (
        f"Rappel : vous avez un rendez-vous avec Dr. {doctor_name} le {appointment.get('appointment_date')} à {appointment.get('appointment_time')}.\n"
        "Merci d'arriver 10 minutes avant l'heure prévue."
    )
    _send_appointment_email("Rappel de rendez-vous", body, user.get("email"))
    publish_event("appointment.reminder_due", {
        "user_id": appointment["user_id"],
        "title": "Rappel de rendez-vous",
        "message": f"Rappel : rendez-vous avec Dr. {doctor_name} le {appointment.get('appointment_date')} à {appointment.get('appointment_time')}.",
    })
...
def delete_appointment(uid: str):
    ...
    publish_event("appointment.cancelled", {
        "user_id": appointment["user_id"],
        "title": "Rendez-vous annulé",
        "message": f"Votre rendez-vous du {appointment.get('appointment_date')} à {appointment.get('appointment_time')} a été annulé.",
    })
    cursor.close()
    conn.close()
...
def confirm_appointment(uid: str):
    ...
    conn.commit()
    publish_event("appointment.confirmed", {
        "user_id": appointment["user_id"],
        "title": "Rendez-vous confirmé",
        "message": f"Votre rendez-vous du {appointment.get('appointment_date')} à {appointment.get('appointment_time')} est confirmé.",
    })
    cursor.close()
    conn.close()
